...
test("finds a phrase and says where it is", async () => {
  await withWorkspace({ "notes.md": "alpha\nthe needle is here\nomega\n" }, async ({ ipcMain, q }) => {
    const result = await find(ipcMain, q, { pattern: "needle" });
...
test("searches only below the folder it was given", async () => {
  await withWorkspace(
    ...
    async ({ ipcMain, q }) => {
      const result = await find(ipcMain, q, { path: q("docs"), pattern: "needle" });
      assert.deepEqual(
...
test("reads only the file types that are turned on", async () => {
  await withWorkspace({ "a.md": "needle", "b.log": "needle" }, async ({ ipcMain, q }) => {
    const result = await find(ipcMain, q, { pattern: "needle" });
    assert.deepEqual(
      result.hits.map((hit) => hit.path),
      ["a.md"],
...
test("matches case when it is told to", async () => {
  await withWorkspace({ "notes.md": "Needle\nneedle\nNEEDLE\n" }, async ({ ipcMain, q }) => {
    const insensitive = await find(ipcMain, q, { pattern: "needle" });
    assert.equal(insensitive.hits.length, 3);

    const sensitive = await find(ipcMain, q, { pattern: "needle", caseSensitive: true });
...
test("takes a regular expression when it is told to", async () => {
  await withWorkspace({ "notes.md": "cat\nmat\ndog\n" }, async ({ ipcMain, q }) => {
    const result = await find(ipcMain, q, { pattern: "[cm]at", regex: true });
...
test("says so when the expression does not compile", async () => {
  await withWorkspace({ "notes.md": "x" }, async ({ ipcMain, q }) => {
    const result = await find(ipcMain, q, { pattern: "[unclosed", regex: true });
...
test("refuses a folder that climbs out of the workspace", async () => {
  await withWorkspace({ "notes.md": "needle" }, async ({ ipcMain, q }) => {
    const result = await find(ipcMain, q, { path: q("../.."), pattern: "needle" });
    assert.equal(result.ok, false);
  });
});
...
test("says when it stopped early", async () => {
  ...
  await withWorkspace(files, async ({ ipcMain, q }) => {
    const result = await find(ipcMain, q, { pattern: "needle" });
    assert.equal(result.ok, true);
