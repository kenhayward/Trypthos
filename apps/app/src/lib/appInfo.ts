...
| Formatting toolbar | In Source view, a button for every markdown construct. Headings act on the current line, character formatting wraps your selection, and a second press removes what the first added. Paste as markdown keeps the structure of text copied from a rendered page or a Word document, and turns a spreadsheet range into a table. Ctrl+Shift+V (Cmd on macOS) does it from the keyboard while your caret is in an editable document. |
...
| Menus | File, Edit, Tools and Help, plus a right-click menu with editing and spelling corrections in every text field. Right-clicking over an editable markdown document also offers Paste as markdown, which converts what you copied instead of taking its plain text - in Live and Source view alike. Settings, About and the release notes are reached from the menus - on macOS, from the Trypthos menu. |
