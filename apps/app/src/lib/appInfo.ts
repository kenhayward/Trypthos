...
| Feature | Description |
| --- | --- |
| Workspace browser | Open as many folders as you like - on this machine or on GitHub - each with its own tree, collapsible from its own row, and its own close button. They reopen next time you start. Right-click a local folder to make a file or folder there, or a file to open it in a focused document-only window. Right-click one to refresh it. The filter box searches every open folder by name, however deep, and takes * and ? wildcards. OneDrive, Google Drive and Dropbox follow in a later release. |
