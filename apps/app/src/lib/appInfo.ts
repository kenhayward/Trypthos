...
| AI chat | Appears once you configure a model, and can be switched off. The chat box grows as you type. Ask about your document, a selection, attached files (picked, or added or dragged from the browser) or a folder you pick in the browser, apply proposed changes, and watch how full the context is. The model is told what kind of file it is looking at, and a model with a reasoning mode can be asked to think first. A reply normally streams as it arrives, and each model can ask for a complete reply instead when its streamed tool calls are incomplete, and has its own reply timeout. A reply lists its tool calls and the model's thinking, each behind a fold. Reading a file works with or without tool calling. Save a conversation under a name, with its attached files' text and its folder's path, and reopen it as it was. See how the latest reply went - time to first token, response time, tokens per second, tokens and context used - and copy it as markdown. Type /commands or /tools in the chat box to see what it can do. A file path a reply mentions is a link that opens it. With a folder attached it can read any enabled file inside it, including one found through a directory listing, and can search or compare files there. It can open a file it found, and create a new one - never replacing an existing file. |
