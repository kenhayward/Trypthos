...
describe("TitleBar", () => {
  it("shows the app name alone when no file is open", () => {
    render(<TitleBar platform="win32" fileName={null} onAbout={noop} />);
    expect(screen.getByText("Trypthos")).toBeDefined();
  });

  it("names the open file, separated by a plain hyphen", () => {
    render(<TitleBar platform="win32" fileName="README.md" onAbout={noop} />);
  ...
  it("carries About, since there is no in-app header any more", async () => {
    ...
    render(<TitleBar platform="win32" fileName={null} onAbout={onAbout} />);
  ...
  it("draws no window controls on macOS", () => {
    render(<TitleBar platform="darwin" fileName={null} onAbout={noop} />);
  ...
  it("reserves room for the traffic lights on macOS, and none elsewhere", () => {
    const { container: mac } = render(<TitleBar platform="darwin" fileName={null} onAbout={noop} />);
    expect((mac.querySelector("header") as HTMLElement).style.paddingLeft).toBe("80px");

    const { container: win } = render(<TitleBar platform="win32" fileName={null} onAbout={noop} />);
  ...
  it("makes the bar draggable and its controls clickable", () => {
    const { container } = render(<TitleBar platform="win32" fileName={null} onAbout={noop} />);
