...
/// What a repository's heading needs, and what it does when it cannot get it.
///
/// The README used to be fetched here too. It is not GitHub's, and every workspace has a home page
/// that can show one now, so it moved to `useReadme` - whose tests hold what these used to about it.
...
function fakes(overrides: { github?: Partial<GitHubBridge> } = {}) {
  const github = {
...
describe("useRepoPage", () => {
  it("fetches the statistics for the workspace it is given", async () => {
    const { github } = fakes();
    const { result } = renderHook(() => useRepoPage("notes", github));
  ...
  it("says why the statistics could not be fetched", async () => {
    const { github } = fakes({
  ...
  it("asks for nothing at all without a workspace", async () => {
    const { github } = fakes();
    const { result } = renderHook(() => useRepoPage(null, github));
  ...
  it("drops an answer for a workspace it has since moved off", async () => {
    const { github } = fakes();
    const { result, rerender } = renderHook(({ id }) => useRepoPage(id, github), {
...
describe("without a shell", () => {
  ...
  it("does not sit loading when there is no bridge", async () => {
    const { result } = renderHook(() => useRepoPage("notes", null));
