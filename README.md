...
| Syntax colouring | Every type is coloured by role - keywords, strings, comments, numbers, types and names - from the same palette as the rest of the app, in light and dark. Fenced code blocks inside a markdown document are coloured by the language on the fence. Grammars load only when a file needs one. |
