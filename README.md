...
| Feature | Description |
| --- | --- |
| Workspace browser | Open several local folders at once. Each has its own row and tree, and a close button that takes its tabs with it; every open folder reopens next time you start. A file is named by its folder as well as its path, so two files with the same name in different folders stay two documents. OneDrive, Google Drive, Dropbox and GitHub follow. |
